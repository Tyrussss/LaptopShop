...
//        out.println("<h1>Servlet DeleteItemCartServlet at " + lectureID  + "</h1>");

        ArrayList<CartItem> list=(ArrayList<CartItem>)request.getSession().getAttribute("lectureList");
        for(int index=0;index<list.size();index++){
            if(list.get(index).getLecture().getLectureID()==lectureID){
                list.remove(index);
                break;
            }
        }
        request.getSession().setAttribute("lectureList", list);// Update list -1 item
